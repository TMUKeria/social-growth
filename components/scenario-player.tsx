...
export function ScenarioPlayer({ returnHref, returnLabel, scenarioTitle, steps }: Props) {
  ...
  function retryStep() { setSelectedChoice(null); setRetriedCurrentStep(true); }
  function moveNext() {
    window.speechSynthesis?.cancel();
    ...
    <main className="min-h-screen bg-[#eef3fa] p-2 sm:p-5">
      <div className="mx-auto max-w-[1400px] overflow-hidden rounded-[1.8rem] bg-white shadow-2xl">
        ...
        <section className="p-5 sm:p-8 lg:p-10">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3" aria-label="선택지">{step.choices.map((choice, index) => { const isSelected = selectedChoice?.id === choice.id; const stateStyle = isSelected ? (choice.is_correct ? "ring-4 ring-green-500" : "ring-4 ring-red-500") : "hover:-translate-y-1 hover:shadow-lg"; return <button className={`min-h-36 rounded-3xl border-[3px] p-5 text-center text-lg font-black transition sm:text-xl ${choiceStyles[index % 3]} ${stateStyle}`} disabled={Boolean(selectedChoice)} key={choice.id} onClick={() => choose(choice)} type="button"><span className="mr-3 text-4xl" aria-hidden="true">{choiceIcons[index % 3]}</span>{choice.text}</button>; })}</div>
          {selectedChoice && <div className={`mt-8 grid items-center gap-5 rounded-3xl border-2 p-6 sm:grid-cols-[auto_1fr_auto] ${selectedChoice.is_correct ? "border-amber-200 bg-amber-50" : "border-red-200 bg-red-50"}`} role="status" aria-live="polite"><span className={`flex size-20 items-center justify-center rounded-full text-5xl font-black text-white ${selectedChoice.is_correct ? "bg-[#ffc72c]" : "bg-red-500"}`} aria-hidden="true">{selectedChoice.is_correct ? "★" : "×"}</span><div><p className="text-xl font-black">{selectedChoice.is_correct ? "잘했어요!" : "한 번 더 생각해 볼까요?"}</p><p className="mt-1 text-lg leading-7">{selectedChoice.feedback_text}</p></div>{selectedChoice.is_correct ? <button className="min-h-14 rounded-2xl bg-[#0754c9] px-6 font-black text-white" onClick={moveNext} type="button">{stepIndex === steps.length - 1 ? "결과 보기" : "다음 상황"}</button> : <button className="min-h-14 rounded-2xl bg-red-600 px-6 font-black text-white" onClick={retryStep} type="button">다시 선택하기</button>}</div>}
        </section>
      </div>
    </main>
  );
